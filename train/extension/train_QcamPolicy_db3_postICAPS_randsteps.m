...
function [Qc,tot_reward,rew_allep] = train_QcamPolicy_db3_postICAPS_randsteps(opts, Q_table_name, Qc)
...
for epoch = 50000 : opts.numEpoch
    ...
    for p = pALL(1:2:end) 
        ...
        % select camera visited in the trajectory
        uniq_cam = unique(ped(:,1));
        randCam = uniq_cam(randi(length(uniq_cam)));  % select a randomCam from visited cams
        index_of_randCam = find( ped(ped(:,1)==randCam,1) );  % find indices of the selected camera
...
%             end
            ...
            if ~isPresent
                %telapse = telapse +1;
                history.te = min(ceil(telapse/1), opts.lte);
            else
                telapse = 0;
                history.te = 1;
            end
