...
function [Qc,tot_reward,rew_allep] = train_db3_postICAPS_3rew_selectNearTr_te2_f2(opts, Q_table_name, Qc)
...
tot_reward = [];
tic_train = tic;
for epoch = 220001 : opts.numEpoch
    ...
    for p = pALL(1:2:end) 
        ...
        if rand < 0.4 % select anywhere
            startIDX = index_of_randCam(randi(len_indices_of_randCam));
        else % select near transition
            startIDX  = tranIDX(randi( length(tranIDX) ))-10;
        end
        
        %disp(startIDX); disp(size(ped)); disp(tranIDX);
        ...
        while(curr_frame <= ped(end,2) )
            ...
            % % take random steps
            if rand < 0.5  %log(epoch)/10
                %r = randi(opts.lte);
                %r = randsample(1:opts.lte,1,true,(1./[1:opts.lte])/sum(1./[1:opts.lte]));
                r = randsample(1:20,1,true,(1./[1:20])/sum(1./[1:20]));
            else
                r = 1;
            end
            
            telapse = telapse+r;
            curr_frame = curr_frame+fpsc*r;
