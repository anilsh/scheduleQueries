function [Qc,tot_reward,rew_allep] = train_QcamPolicy_db4_icaps(opts, Q_table_name, Qc)
...
d = 8;       % divide image in dxd region
opts.d = d;
opts.lte = 300; % maximum length of telapse
if nargin < 3
...
load('./data/ann_MCT_dataset4_pidWise.mat');
...
for epoch = 1 : opts.numEpoch
    ...
    for p = pALL(1:2:end) 
        ...
        if length(uniq_cam) < 2
            continue;
        end
        
        startIDX = index_of_randCam(randi(len_indices_of_randCam)); %randi( min(100,size(ped,1)-100) );
        %startIDX = randi( min(100,size(ped,1)-100) );
        ...
        %while(numImages <= alltime && num_steps < episode_length)
...
%             end
            ...
            if ~isPresent
                telapse = telapse +1;
                history.te = min(ceil(telapse/3), opts.lte);
            else
                telapse = 0;
                history.te = 1;
            end
        ...
        totalReward_ped = [totalReward_ped, 1*length(pos_labels)-1*length(neg_labels) ];
