function [tot_reward,rew_allep,results_sel,pred_allP] = test_QcamPolicy_db3_errREIDicaps(opts, Qc)
...
for epoch = 1 : 1
    ...
    for p = pALL(2:2:end)
        ...
        while(curr_frame <= ped(end,2))
            ...
            if rand < 0.1  && (inc == 0 || req_only_ict==0) % flip presence information
                if isPresent == 1
                    isPresent = 0;
                    bbox_pt_n = [];
                else
