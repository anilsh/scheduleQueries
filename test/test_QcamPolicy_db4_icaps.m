function [tot_reward,rew_allep,results_sel,pred_allP] = test_QcamPolicy_db4_icaps(opts, Qc)
...
for epoch = 1 : 1
    ...
    for p = pALL(2:2:end) 
        ...
        %while(numImages <= alltime)
        while(curr_frame <= alltime) %ped(end,2))
            ...
            if isPresent == 1 && isempty(bbox_pt_n)
                disp('');
            end
            if isPresent == 0 && ~isempty(bbox_pt_n)
                disp('');
            end
            ...
            if ~isPresent
                telapse = telapse +1;
                history.te = min(ceil(telapse/4), opts.lte);
                % optimal te is 4
            else
                telapse = 0;
                history.te = 1;
        ...
        if isnan(results_sel{rs_cnt}.prec)
            disp('');
        end
        rs_cnt = rs_cnt + 1;
